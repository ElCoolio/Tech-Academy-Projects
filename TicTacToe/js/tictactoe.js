...
function placeXOrO(squareNumber){
    //This condition ensure a squanre hasne't been selected already
    //the .some() method is used to cheack each element of the selectSquare array
    //to see if it contains the square number clicked on.
    if (!selectedSquares.some(element=> element.includes(squareNumber))){
        //This variable checks who's turn it is.
        let select = document.getElementById(squareNumber);
        //This condition checks who's turn it is.
        if (activePlayer === "X"){
            //If activePlayer is equal to "X", the x.png is place in HTML
            select.style.backgroundImage = 'url("images/x.png")';
            //Active player may only be 'x' or 'o' so if it is not "X" it must be "O"
        } else {
        ...
        if(activePlayer ==='O'){
            //This feature disables clicking for the computer's turn.
            disableClick();
            //This function waits 1 second before the compute places an imaage and enables click.
            setTimeout(function () {computersTurn(); }, 1000);
        }
        //returning ture is need for our computerTurn function to work.
        return true;
    }
    ...
    function computersTurn() {
        ...
        let pickASquare;
        //This conditiont allows our while loop to keep trying if a square is selected already.
        while(!success){
...
//drawLine() function is called to draw a lined on the screen if the condition is met.
